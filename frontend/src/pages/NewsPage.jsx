...
const NewsPage = () => {
  ...
  const filteredNewsItems = newsItems.filter(news => {
    ...
    return (
      ...
      (news.create_user && news.create_user.toLowerCase().includes(query))
    );
  });
            ...
            {filteredNewsItems.length > 0 ? (
              filteredNewsItems.map(news => (
                ...
                >
                  ...
                  <div className="relative w-full h-[200px] overflow-hidden rounded-t-2xl bg-gradient-to-br from-gray-200 to-gray-300">
                    <img
                      className="h-full w-full object-cover transition-all duration-700 group-hover:scale-110"
                      src={`${newsService.BASE_BACKEND_URL}/NewsImage/${news.image}`}
                      alt={news.title}
                      onError={(e) => {
                  ...
                  <div className="flex-grow flex flex-col p-5 bg-gradient-to-br from-white to-gray-50/50 text-left min-h-[100px] z-20 backdrop-blur-sm">
                    ...
                    <p className="text-sm text-gray-600 mb-2">
                      {news.create_user || 'Unknown Source'} &bull; {new Date(news.created_at).toLocaleDateString()}
                    </p>
